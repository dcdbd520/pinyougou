...
public class ItemSearchServiceImpl implements ItemSearchService {
    ...
    private Map searchList(Map searchMap) {
        ...
        if (!"".equals(searchMap.get("price"))&&searchMap.get("price")!=null){
            ...
            if (!price[0].equals("0")){//价格区间起点不等于0
                Criteria filterCriteria = new Criteria("item_price").greaterThanEqual(price[0]);
                FilterQuery filterQuery = new SimpleFilterQuery(filterCriteria);
                query.addFilterQuery(filterQuery);
            }
            if (!price[1].equals("*")){//价格区间终点不等于*
        ...
        query.setOffset((pageNo-1)*pageSize);//从第几条开始查
        query.setRows(pageSize);//每页数据条数
        //1.7排序查询
        String sortValue = (String) searchMap.get("sort");//获取排序方式
        String sortField = (String) searchMap.get("sortField");//获取排序的字段
