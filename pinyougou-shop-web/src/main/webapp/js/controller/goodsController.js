		...
		goodsService.findOne(id).success(
			function(response) {
                ...
                $scope.entity.goodsDesc.customAttributeItems =
                    JSON.parse($scope.entity.goodsDesc.customAttributeItems);//扩展属性与下面代码冲突
    ...
    $scope.$watch('entity.goods.typeTemplateId',function (newValue, oldValue) {
        typeTemplateService.findOne(newValue).success(
            function (response) {
                ...
                $scope.typeTemplate.brandIds=JSON.parse($scope.typeTemplate.brandIds)//品牌列表String转JSON
                //获取规格属性
    ...
    $scope.updateSpecAttribute=function($event,name,value){
        ...
        if(object!=null){
            if($event.target.checked ){
                ...
                object.attributeValue.splice( object.attributeValue.indexOf(value ) ,1);//移除项
                //如果选项都取消了，将此条记录移除
